...
# PWCNet model trained on the FlyingChairs dataset
# Source: https://github.com/NVlabs/PWC-Net
model = torch.load("PWCNet.pth")
...
for i in range(frame_count):
    ...
    # Extract optical flow features using PWCNet model
    if prev_frame is not None:
        with torch.no_grad():
            output = model(prev_frame, frame)
            output = output["flow"].squeeze(dim=0)
